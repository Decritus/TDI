...
//    typedef struct { IndexT row, col; } CoordT; // just for a few funcions that return a coordinate
  ...
  private: // data
    ...
    C_Matrix * superMat;        // If this is a matrix => NULL. Else, if this //super matriz
    ...
    bool       fail;		//errores este fail nos dice si hemos tenido algun error entonces si este fail esta activo ha habido fallos
  ...
  public: // methods
    ...
    C_Matrix (const IndexT firstRow, const IndexT lastRow, //construcyor de matriz
    ...
    C_Matrix (C_Matrix & superMatrix,			//constructor de submatriz
    ...
    void Free (void);		//liberacion de memoria
    ...
    bool In (const IndexT row, const IndexT col) const; //devuelve si las coordenadas estan dentro de la matriz

    // Resets fail status to false. Needed after an error detection.
    void Clear (void);

    // Access to a matrix element: overloading ()//sobre carga de operadoes
